...
namespace AITIssueTracker.API.v0._3_DAL
{
    public class IssueContext : PsqlMaster
    {
        ...
        public async Task<List<User>> SelectUsersOfIssueAsync(Guid issueId)
        {
            return await ExecuteSqlAsync(async (cmd) =>
            {
                string SQL_SELECT_USERS_OF_ISSUE_BY_ID = "select * from \"issue_user\" where issue_id=@issue_id;";
                cmd.CommandText = SQL_SELECT_USERS_OF_ISSUE_BY_ID;
                cmd.Parameters.Add("@issue_id", NpgsqlDbType.Uuid).Value = issueId;
