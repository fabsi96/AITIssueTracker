...
namespace AITIssueTracker.API.v0._3_DAL
{
    public class FeatureContext : PsqlMaster
    {
        ...
        public async Task<List<User>> SelectUsersOfFeatureAsync(Guid featureId)
        {
            return await ExecuteSqlAsync(async (cmd) =>
            {
                string SQL_SELECT_USERS_OF_FEATURE_BY_ID = "select * from \"feature_user\" where feature_id=@feature_id;";
                cmd.CommandText = SQL_SELECT_USERS_OF_FEATURE_BY_ID;
                cmd.Parameters.Add("@feature_id", NpgsqlDbType.Uuid).Value = featureId;
