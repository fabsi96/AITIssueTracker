...
namespace AITIssueTracker.API.v0._2_Manager
{
    public class IssueManager
    {
        ...
        {
            var issueToUpdate = new Issue(issue) { Id = issueId };
            var originalIssue = await DbContext.SelectIssueAsync(issueId);
            var issueUsers = await DbContext.SelectUsersOfIssueAsync(issueId);
