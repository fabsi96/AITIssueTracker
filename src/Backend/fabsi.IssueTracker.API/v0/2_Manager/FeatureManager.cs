...
namespace AITIssueTracker.API.v0._2_Manager
{
    public class FeatureManager
    {
        ...
        {
            var featureToSave = new Feature(feature);
        ...
        {
            var projectFeatures = await DbContext.SelectFeaturesByProjectTitleAsync(projectId);
        ...
        {
            var projectOfFeature = await DbContext.SelectProjectOfFeatureAsync(featureId);
        ...
        {
            var targetFeature = await DbContext.SelectFeatureByIdAsync(featureId);
            var featureUsers = await DbContext.SelectUsersOfFeatureAsync(featureId);
